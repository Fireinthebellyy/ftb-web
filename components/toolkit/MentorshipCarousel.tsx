...
export function MentorshipCarousel({ mentorId }: { mentorId: string | undefined }) {
  ...
  return (
    <div className="mb-8 -mx-[6px] md:mx-auto md:w-full md:px-4 max-w-[1646px]">
      {/* Wrapper: h-[180px] on mobile, md:h-[166px] on desktop. flex-row always */}
      <div className="bg-[#ff5e14] rounded-2xl py-1 px-2 sm:p-5 flex flex-row items-center gap-2 md:gap-6 shadow-md relative overflow-visible h-[180px] md:h-[166px] w-full mx-auto">
            ...
            <CarouselContent className="h-full">
              ...
              {hasMentor && (
                <CarouselItem className="h-full flex items-center">
                  <div className="bg-white rounded-2xl p-3 md:px-6 md:py-4 flex flex-col justify-center shadow-sm w-full h-[171px] md:h-full shrink-0">
              ...
              {slides.map((slide) => (
                <CarouselItem key={slide.id} className="h-full flex items-center">
                  <div className="bg-white rounded-2xl overflow-hidden shadow-sm w-full h-[171px] md:h-full flex flex-col justify-center shrink-0">
                    {slide.imageUrl ? (
                      <>
                        <img src={slide.imageUrl} alt={slide.title} className="w-full h-full object-cover md:hidden" />
                        <div className="hidden md:flex flex-row items-center gap-6 px-6 py-4 h-full">
                          <div className="h-20 w-20 shrink-0 block">
                          ...
                          <div className="flex flex-col justify-center min-w-0 h-full">
                            <h3 className="text-2xl font-bold text-gray-900 mb-1.5 leading-tight tracking-tight truncate">
                              {slide.title}
                            </h3>
                            {slide.description && (
                              <p className="text-sm font-medium text-gray-600 leading-relaxed max-w-3xl line-clamp-2">
                                {slide.description}
                              </p>
                            )}
                          </div>
