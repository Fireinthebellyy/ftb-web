...
// Session schema for creating/editing a sprint session
...
const mentorSchema = z.object({
  ...
  sprintMentorId: z.string().optional().nullable(),
  ...
  orderIndex: z.coerce.number().int().min(0).default(0),
}).refine(data => data.sprintMentorId || data.name?.trim(), {
  message: "Mentor name is required when not selecting a sprint mentor",
  path: ["name"],
});
...
interface SprintSessionContent {
  id: string;
  sessionId: string;
...
interface SprintSessionResource {
  id: string;
  contentId: string;
  name: string;
  url: string;
...
export default function SprintSessionManager({
  ...
  const [editingSession, setEditingSession] = useState<SprintSession | null>(null);
  const [isAddingSession, setIsAddingSession] = useState(false);
  const [contentEditDialogOpen, setContentEditDialogOpen] = useState(false);
  const [editingContent, setEditingContent] = useState<SprintSessionContent | null>(null);
  const [isAddingContent, setIsAddingContent] = useState(false);
  const [selectedSessionForContent, setSelectedSessionForContent] = useState<SprintSession | null>(null);
  const [mentorEditDialogOpen, setMentorEditDialogOpen] = useState(false);
  const [editingMentor, setEditingMentor] = useState<SprintSessionMentor | null>(null);
  const [isAddingMentor, setIsAddingMentor] = useState(false);
  const [selectedContentForMentor, setSelectedContentForMentor] = useState<SprintSessionContent | null>(null);
  const [resourceEditDialogOpen, setResourceEditDialogOpen] = useState(false);
  const [editingResource, setEditingResource] = useState<SprintSessionResource | null>(null);
  const [isAddingResource, setIsAddingResource] = useState(false);
  const [selectedContentForResource, setSelectedContentForResource] = useState<SprintSessionContent | null>(null);
  ...
  const [sprintMentors, setSprintMentors] = useState<SprintMentor[]>([]);
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.get(`/api/admin/sprints/${sprintId}/sessions`);
  ...
  useEffect(() => {
    ...
    const fetchSprintDetails = async () => {
      ...
      try {
        ...
        if (active) {
          setSprintMentors(response.data.mentors || []);
        }
      } catch (error) {
        console.error("Error fetching sprint details for mentors:", error);
      }
    };
    fetchSprintDetails();
    return () => {
  ...
  const handleEditSession = (session: SprintSession) => {
  ...
  const handleSaveSession = async (data: SessionFormValues) => {
    try {
      if (editingSession) {
        await axios.put(`/api/admin/sprint-sessions/${editingSession.id}`, data);
  ...
  const handleDeleteSession = async (id: string, title: string) => {
    ...
    try {
      await axios.delete(`/api/admin/sprint-sessions/${id}`);
  ...
  const handleEditContent = (session: SprintSession, content: SprintSessionContent) => {
  ...
  const handleAddContent = (session: SprintSession, sectionType: string) => {
    setSelectedSessionForContent(session);
  ...
  const handleSaveContent = async (data: SessionContentFormValues) => {
    ...
    try {
      ...
      if (editingContent) {
        await axios.put(`/api/admin/sprint-session-contents/${editingContent.id}`, payload);
        toast.success("Content updated successfully!");
      } else {
        await axios.post(`/api/admin/sprint-sessions/${selectedSessionForContent.id}/content`, payload);
  ...
  const handleDeleteContent = async (content: SprintSessionContent, title: string) => {
    if (!confirm(`Are you sure you want to delete "${title}"? This action cannot be undone.`)) {
      return;
    }
    try {
      await axios.delete(`/api/admin/sprint-session-contents/${content.id}`);
  ...
  const handleToggleLock = async (content: SprintSessionContent) => {
    try {
      await axios.put(`/api/admin/sprint-session-contents/${content.id}`, {
        isUnlocked: !content.isUnlocked,
      });
  ...
  const handleEditMentor = (content: SprintSessionContent, mentor: SprintSessionMentor) => {
    setSelectedContentForMentor(content);
    setEditingMentor(mentor);
    setIsAddingMentor(false);
    mentorForm.reset({
      contentId: content.id,
      sprintMentorId: mentor.sprintMentorId ?? "",
      name: mentor.name ?? "",
      role: mentor.role ?? "",
  ...
  const handleAddMentor = (content: SprintSessionContent) => {
    setSelectedContentForMentor(content);
  ...
  const handleSaveMentor = async (data: MentorFormValues) => {
    ...
    try {
      if (editingMentor) {
        await axios.put(`/api/admin/sprint-session-mentors/${editingMentor.id}`, data);
        toast.success("Mentor updated successfully!");
      } else {
        await axios.post("/api/admin/sprint-session-mentors", data);
  ...
  const handleDeleteMentor = async (mentor: SprintSessionMentor, name: string) => {
    if (!confirm(`Are you sure you want to delete mentor "${name}"? This action cannot be undone.`)) {
      return;
    }
    try {
      await axios.delete(`/api/admin/sprint-session-mentors/${mentor.id}`);
  ...
  const handleEditResource = (content: SprintSessionContent, resource: SprintSessionResource) => {
  ...
  const handleAddResource = (content: SprintSessionContent) => {
    setSelectedContentForResource(content);
  ...
  const handleSaveResource = async (data: ResourceFormValues) => {
    ...
    try {
      if (editingResource) {
        await axios.put(`/api/admin/sprint-session-resources/${editingResource.id}`, data);
        ...
        if (data.url && data.url.startsWith('[')) {
          ...
          const promises = multipleResources.map((resource: any) =>
            axios.post("/api/admin/sprint-session-resources", {
              ...data,
          ...
          await axios.post("/api/admin/sprint-session-resources", data);
  ...
  const handleDeleteResource = async (resource: SprintSessionResource, name: string) => {
    if (!confirm(`Are you sure you want to delete resource "${name}"? This action cannot be undone.`)) {
      return;
    }
    try {
      await axios.delete(`/api/admin/sprint-session-resources/${resource.id}`);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[900px]">
        <DialogHeader>
          <DialogTitle>Manage Sprint Sessions: {sprintTitle}</DialogTitle>
        </DialogHeader>

        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <p className="text-muted-foreground text-sm">
              Add and manage sprint sessions
            </p>
            ...
            <Form {...mentorForm}>
              <form onSubmit={mentorForm.handleSubmit(handleSaveMentor)} className="space-y-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Link to Sprint Mentor (Optional)</FormLabel>
                      <FormControl>
                        {sprintMentors.length === 0 ? (
                          <p className="text-xs text-muted-foreground py-2 px-3 border rounded bg-gray-50">
                            No mentors found for this sprint. Add mentors in the sprint&apos;s &quot;Meet your mentors&quot; section first, then they will appear here.
                          </p>
                        ) : (
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              field.onChange(val);
                              if (val) {
                                const selected = sprintMentors.find(m => m.id === val);
                                if (selected) {
                          ...
                          >
                            <option value="">-- Select Sprint Mentor (or fill details below manually) --</option>
                            {sprintMentors.map((m) => (
