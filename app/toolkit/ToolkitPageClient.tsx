...
export default function ToolkitPageClient({
  ...
  //   { label: settings?.toolkitSprintsTabLabel ?? "Sprints", value: "sprints" },
  ...
  const { data: sprintsData = [], isLoading: sprintsLoading } = useQuery<SprintCard[]>({
    queryKey: ["sprints"],
    queryFn: async () => {
      try {
  ...
  if (sessionPending || !session || isLoading || cohortsLoading || sprintsLoading) {
    return (
      ...
      <div className="container mx-auto px-4 py-6">
                  ...
                  isAdmin && sprintsData.length > 0 && (
                    <>
                      {sprintsData.map((sprint) => (
                        <div
                          key={sprint.id}
                          onClick={() => router.push(`/toolkit/sprints/${sprint.id}`)}
                        ...
                        >
                          <div className="relative">
                            {sprint.cardImageUrl || sprint.coverImageUrl ? (
                              <img
                                src={sprint.cardImageUrl || sprint.coverImageUrl}
                                alt={sprint.title}
                              ...
                              <div className="w-full aspect-[16/10] bg-orange-50 flex items-center justify-center border-b">
                                <span className="text-[#ff5e14] font-bold text-lg">Sprint</span>
                              </div>
                            )}
                          ...
                          <div className="p-4">
                            ...
                            {sprint.outerSubtitle && (
                              <p className="mt-1 italic text-sm font-medium text-gray-500 leading-snug line-clamp-2">
                                {sprint.outerSubtitle}
                              </p>
                            )}

                            {sprint.startDate && (
                              <div className="mt-3 flex items-center gap-1.5 text-[11px] font-bold text-emerald-700 uppercase tracking-wider bg-emerald-50 w-fit px-2.5 py-1 rounded-lg border border-emerald-100">
                            ...
                            <div className="flex gap-2 items-center pt-1.5">
                              ...
                              <button
                                ...
                                onClick={async (e) => {
                                  e.stopPropagation();
                                  const url = `${window.location.origin}/toolkit/sprints/${sprint.id}`;
                                  try {
                            ...
                            {sprint.highlights && sprint.highlights.length > 0 && (
        ...
        {/* {isError ? (
          ...
          sprintsData.length === 0 ? (
            <div className="rounded-lg border bg-white py-12 text-center">
              <h3 className="mb-2 text-lg font-semibold text-gray-600">No active sprints found</h3>
              <p className="text-gray-500">Check back soon for upcoming sprint programs!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {sprintsData.map((sprint) => (
