...
function normalizePublishAt(publishAt?: string): string | "" | undefined {
  if (!publishAt) {
    return publishAt;
  }
  ...
  return parsedPublishAt.toISOString();
}
...
function extractApiErrorMessage(error: unknown): string {
  ...
  if (Array.isArray(payloadError)) {
    const fieldErrors = payloadError
      .map((item) => {
  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      ...
      const payload = {
        ...
        publishAt: normalizePublishAt(restData.publishAt),
        tags:
          data.tags
            ?.split(",")
            .map((t) => t.trim())
