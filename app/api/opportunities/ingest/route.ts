...
import {
  buildOpportunityInsertValues,
  getOpportunityIngestUserId,
  opportunityIngestBatchSchema,
} from "@/lib/opportunities-ingest";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const records = opportunityIngestBatchSchema.parse(body);
    const ingestUserId = await getOpportunityIngestUserId();
    const values = await buildOpportunityInsertValues(records, ingestUserId);
    ...
    console.error("Error bulk ingesting opportunities:", error);
    return NextResponse.json(
      { error: "Internal Server Error" },
      { status: 500 }
    );
