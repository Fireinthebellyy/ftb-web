...
export async function GET(
  ...
  try {
    ...
    const contents = await db
      ...
      .orderBy(asc(sprintSessionContents.orderIndex));

    activityStatus = 200;
    return NextResponse.json(contents);
  } catch (error) {
    activityError = error;
    console.error("Error fetching sprint session contents:", error);
    activityStatus = 500;
    ...
    void logAdminActivity({
      request,
      action: "admin.sprints.sessions.content.list",
...
export async function POST(
  ...
  try {
    ...
    activityError = error;
    console.error("Error creating sprint session content:", error);
    activityStatus = 500;
    return NextResponse.json(
    ...
    void logAdminActivity({
      request,
      action: "admin.sprints.sessions.content.create",
      statusCode: activityStatus,
      success: activityStatus >= 200 && activityStatus < 300,
      adminUserId: activityAdminUserId,
      entityType: "sprint_session",
