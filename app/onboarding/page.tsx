            ...
            base.push(
                {
                    ...
                    description: "City or state helps us surface nearby and remote options.",
                },
                {
                    id: "education",
    ...
    const goNext = async () => {
        if (stepIndex < steps.length - 1) {
            ...
            await saveProfile.mutateAsync({
                ...
                locationType: answers.role === "student" ? answers.locationType : undefined,
                locationValue: answers.role === "student" ? answers.locationValue : undefined,
        ...
        <div className="h-full grow bg-gradient-to-b from-neutral-50 via-orange-50/40 to-white">
            <div className="mx-auto max-w-6xl px-4 py-12 md:py-8">
                <div className="mb-8 flex flex-col items-center gap-3 text-center">
                    <Image src="/images/fire-logo.png" alt="Fire in the Belly Logo" width={80} height={80} className="mb-4 object-contain" />
                    ...
                    <p className="text-muted-foreground max-w-2xl text-sm md:text-base">
                        Answer a few quick questions so we can surface the right internships,
                        scholarships, and resources before anyone else.
                    </p>
                ...
                <div className="grid gap-6 lg:grid-cols-[1.7fr_1fr]">
                                        ...
                                        <div className="space-y-4">
                                            <div className="space-y-3">
                                                ...
                                                <select
                                                    ...
                                                    onKeyDown={handleKeyDown}
                                                    className="flex h-11 w-full items-center justify-between rounded-md border border-gray-300 bg-white px-3 py-2 text-sm ring-offset-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                                                    disabled={isBusy}
                                                >
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                Select your state first, then optionally add your city. This helps us surface nearby events and remote-friendly options.
                                            </p>
                                        </div>
                                    )}
                                        ...
                                        <div className="grid gap-3 md:grid-cols-2">
                                            {educationLevels.map((level) => (
                                                <SelectableButton
                                                    ...
                                                    onClick={() =>
                                                        setAnswers((prev) => ({ ...prev, educationLevel: level }))
                                                    }
                                                    onKeyDown={handleKeyDown}
                                                    disabled={isBusy}
                                                />
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {answers.fieldOfStudy === "Other" && (
                                                <Input
                                                    ...
                                                    onChange={(e) =>
                                                        setAnswers((prev) => ({ ...prev, fieldOther: e.target.value }))
                                                    }
                                                    onKeyDown={handleKeyDown}
                                                    disabled={isBusy}
                                                />
                                        ...
                                        <div className="grid gap-3 md:grid-cols-2">
                                            {struggleOptions.map((item) => (
                                                <SelectableButton
                                                    ...
                                                    onKeyDown={handleKeyDown}
                                                    icon={answers.struggles.includes(item) ? <Check /> : undefined}
                                                    disabled={isBusy}
                                                />
                                            ))}
                                        </div>
                                    ...
                                    {currentStep.id === "wrap" && (
                                        <div className="space-y-4">
                                            <div className="rounded-lg border border-dashed border-orange-200 bg-orange-50/60 p-4 text-sm text-gray-800">
                                                <p className="font-medium text-orange-700">You&apos;re set!</p>
                                                <p className="text-muted-foreground mt-1">
                                                    We&apos;ll tune your feed to match these picks. You can always edit
                                            ...
                                            <div className="grid gap-3 md:grid-cols-2">
                                                <SummaryItem
                                                    title="Role"
                                                    value={answers.role ? titleCase(answers.role) : "Not set"}
                                                />
                                                {answers.role === "student" && (
                                                    <>
                                                        ...
                                                        <SummaryList title="Struggles" items={answers.struggles} />
                                                    </>
                                                )}
                                            </div>
                                        </div>
                    ...
                    <Card className="border-orange-100/60 bg-white/70 shadow-sm backdrop-blur-sm max-h-[600px] overflow-y-auto">
                        <CardHeader>
                        ...
                        <CardContent className="space-y-4 text-sm">
                            ...
                            {answers.role === "student" && (
                                <>
                                    ...
                                    />
                                    <PreviewList label="Opportunities" items={answers.opportunities} />
                            ...
                            {answers.role === "society" && (
                                <div className="rounded-md bg-orange-50 p-3 text-xs text-orange-700">
                                    Want student questions? Switch to Student above to unlock the full flow.
                                </div>
                            )}
                            <Separator />
                            <div className="flex items-center justify-between text-xs text-muted-foreground">
                                <span>{profileQuery.isFetching ? "Loading your picks..." : "Ready when you are"}</span>
...
function OptionCard({
    ...
    return (
        <button
            ...
            autoFocus={autoFocus}
            className={cn(
                "w-full rounded-xl border bg-white p-5 text-left shadow-xs transition-all hover:-translate-y-0.5 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-200 disabled:cursor-not-allowed disabled:opacity-60",
                selected
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-3">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
                {items.length === 0 ? (
                    ...
                    items.map((item) => (
                        <Badge key={item} variant="secondary" className="bg-orange-50 text-orange-700">
                            {item}
                        </Badge>
                    ))
                )}
